...
module Translation
  ...
  def translate
    call_deepl(self, self.language, self.to_language)
  end

  def translate!
    self.replace translate
  end
  ...
  def call_deepl(text, language = self.language, to_lan = self.to_language)
